...
/datum/mutation_upgrade/defense/hive_toughness/on_structure_update(previous_amount, new_amount)
	. = ..()
...
/datum/mutation_upgrade/defense/hive_toughness/proc/on_movement(datum/source, atom/old_loc, movement_dir, forced, list/old_locs)
	SIGNAL_HANDLER
...
/datum/mutation_upgrade/defense/hive_toughness/proc/get_armor(structure_count, include_initial = TRUE)
...
/datum/mutation_upgrade/offense/web_yank/get_desc_for_alert(new_amount)
	if(!new_amount)
		return ..()
...
/datum/mutation_upgrade/offense/web_yank/on_gain()
	. = ..()
...
/datum/mutation_upgrade/offense/web_yank/on_loss()
	. = ..()
	var/datum/action/ability/activable/xeno/leash_ball/leash_ability = xenomorph_owner.actions_by_path[/datum/action/ability/activable/xeno/leash_ball]
	if(!leash_ability)
		return
...
/datum/mutation_upgrade/offense/web_yank/on_structure_update(previous_amount, new_amount)
	. = ..()
	var/datum/action/ability/activable/xeno/leash_ball/leash_ability = xenomorph_owner.actions_by_path[/datum/action/ability/activable/xeno/leash_ball]
	if(!leash_ability)
		return
...
/datum/mutation_upgrade/offense/web_yank/proc/get_multiplier(structure_count, include_initial = TRUE)
...
/datum/mutation_upgrade/utility/incubator/get_desc_for_alert(new_amount)
	if(!new_amount)
		return ..()
	return "Birth Spiderling's cooldown is set to [PERCENT(1 + get_multiplier(new_amount))]% of its original cooldown."

/datum/mutation_upgrade/utility/incubator/on_gain()
	. = ..()
...
/datum/mutation_upgrade/utility/incubator/on_loss()
	. = ..()
	var/datum/action/ability/xeno_action/create_spiderling/spiderling_ability = xenomorph_owner.actions_by_path[/datum/action/ability/xeno_action/create_spiderling]
	if(!spiderling_ability)
		return
...
/datum/mutation_upgrade/utility/incubator/on_structure_update(previous_amount, new_amount)
	. = ..()
	var/datum/action/ability/xeno_action/create_spiderling/spiderling_ability = xenomorph_owner.actions_by_path[/datum/action/ability/xeno_action/create_spiderling]
	if(!spiderling_ability)
		return
...
/// Returns the multiplier of Birth Spiderling's initial ability cooldown to add to it.
/datum/mutation_upgrade/utility/incubator/proc/get_multiplier(structure_count, include_initial = TRUE)
	return (include_initial ? multiplier_initial : 0) + (multiplier_per_structure * structure_count)
