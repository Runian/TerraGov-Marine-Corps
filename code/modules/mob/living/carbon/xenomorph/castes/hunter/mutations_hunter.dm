...
/datum/mutation_upgrade/defense/fleeting_mirage/get_desc_for_alert(new_amount)
	if(!new_amount)
		return ..()
...
/datum/mutation_upgrade/defense/fleeting_mirage/on_gain()
	. = ..()
...
/datum/mutation_upgrade/defense/fleeting_mirage/on_loss()
	. = ..()
	var/datum/action/ability/xeno_action/mirage/mirage_ability = xenomorph_owner.actions_by_path[/datum/action/ability/xeno_action/mirage]
	if(!mirage_ability)
		return
...
/datum/mutation_upgrade/defense/fleeting_mirage/proc/on_update_health(datum/source)
...
/datum/mutation_upgrade/defense/fleeting_mirage/proc/remove_illusion()
	var/datum/action/ability/xeno_action/mirage/mirage_ability = xenomorph_owner.actions_by_path[/datum/action/ability/xeno_action/mirage]
	if(!mirage_ability)
		return
	QDEL_NULL(mirage_ability.prioritized_illusion)
...
/datum/mutation_upgrade/defense/fleeting_mirage/proc/get_threshold(structure_count, include_initial = TRUE)
...
/datum/mutation_upgrade/defense/splitting_mirage/get_desc_for_alert(new_amount)
	if(!new_amount)
		return ..()
...
/datum/mutation_upgrade/defense/splitting_mirage/on_gain()
	. = ..()
	var/datum/action/ability/xeno_action/mirage/mirage_ability = xenomorph_owner.actions_by_path[/datum/action/ability/xeno_action/mirage]
	if(!mirage_ability)
		return
...
/datum/mutation_upgrade/defense/splitting_mirage/on_loss()
	. = ..()
	var/datum/action/ability/xeno_action/mirage/mirage_ability = xenomorph_owner.actions_by_path[/datum/action/ability/xeno_action/mirage]
	if(!mirage_ability)
		return
...
/datum/mutation_upgrade/defense/splitting_mirage/on_structure_update(previous_amount, new_amount)
	. = ..()
	var/datum/action/ability/xeno_action/mirage/mirage_ability = xenomorph_owner.actions_by_path[/datum/action/ability/xeno_action/mirage]
	if(!mirage_ability)
		return
...
/datum/mutation_upgrade/defense/splitting_mirage/proc/get_length(structure_count)
...
/datum/mutation_upgrade/defense/cloaking_mirage/get_desc_for_alert(new_amount)
	if(!new_amount)
		return ..()
...
/datum/mutation_upgrade/defense/cloaking_mirage/on_gain()
	. = ..()
	var/datum/action/ability/xeno_action/mirage/mirage_ability = xenomorph_owner.actions_by_path[/datum/action/ability/xeno_action/mirage]
	if(!mirage_ability)
		return
...
/datum/mutation_upgrade/defense/cloaking_mirage/on_loss()
	. = ..()
	var/datum/action/ability/xeno_action/mirage/mirage_ability = xenomorph_owner.actions_by_path[/datum/action/ability/xeno_action/mirage]
	if(!mirage_ability)
		return
...
/datum/mutation_upgrade/defense/cloaking_mirage/on_structure_update(previous_amount, new_amount)
	. = ..()
	var/datum/action/ability/xeno_action/mirage/mirage_ability = xenomorph_owner.actions_by_path[/datum/action/ability/xeno_action/mirage]
	if(!mirage_ability)
		return
...
/// Returns the amount of deciseconds to increase Mirage's illusion lifespan by.
/datum/mutation_upgrade/defense/cloaking_mirage/proc/get_length(structure_count)
	return length_per_structure * structure_count
...
/datum/mutation_upgrade/offense/debilitating_strike/get_desc_for_alert(new_amount)
	if(!new_amount)
		return ..()
...
/datum/mutation_upgrade/offense/debilitating_strike/on_gain()
	. = ..()
...
/datum/mutation_upgrade/offense/debilitating_strike/on_loss()
	. = ..()
	var/datum/action/ability/xeno_action/stealth/stealth_ability = xenomorph_owner.actions_by_path[/datum/action/ability/xeno_action/stealth]
	if(!stealth_ability)
		return
...
/datum/mutation_upgrade/offense/debilitating_strike/on_structure_update(previous_amount, new_amount)
	. = ..()
	var/datum/action/ability/xeno_action/stealth/stealth_ability = xenomorph_owner.actions_by_path[/datum/action/ability/xeno_action/stealth]
	if(!stealth_ability)
		return
...
/datum/mutation_upgrade/offense/debilitating_strike/proc/get_multiplier(structure_count, include_initial = TRUE)
...
/datum/mutation_upgrade/offense/ambush/get_desc_for_alert(new_amount)
	if(!new_amount)
		return ..()
...
/datum/mutation_upgrade/offense/ambush/on_gain()
	. = ..()
	var/datum/action/ability/xeno_action/stealth/stealth_ability = xenomorph_owner.actions_by_path[/datum/action/ability/xeno_action/stealth]
	if(!stealth_ability)
		return
...
/datum/mutation_upgrade/offense/ambush/on_loss()
	. = ..()
	var/datum/action/ability/xeno_action/stealth/stealth_ability = xenomorph_owner.actions_by_path[/datum/action/ability/xeno_action/stealth]
	if(!stealth_ability)
		return
...
/datum/mutation_upgrade/offense/ambush/on_structure_update(previous_amount, new_amount)
	. = ..()
	var/datum/action/ability/xeno_action/stealth/stealth_ability = xenomorph_owner.actions_by_path[/datum/action/ability/xeno_action/stealth]
	if(!stealth_ability)
		return
...
/datum/mutation_upgrade/offense/ambush/proc/get_ap(structure_count, include_initial = TRUE)
...
/datum/mutation_upgrade/offense/maul/on_gain()
	. = ..()
...
/datum/mutation_upgrade/offense/maul/on_loss()
	. = ..()
	var/datum/action/ability/activable/xeno/pounce/pounce_ability = xenomorph_owner.actions_by_path[/datum/action/ability/activable/xeno/pounce]
	if(!pounce_ability)
		return
...
/datum/mutation_upgrade/offense/maul/on_structure_update(previous_amount, new_amount)
	. = ..()
	var/datum/action/ability/activable/xeno/pounce/pounce_ability = xenomorph_owner.actions_by_path[/datum/action/ability/activable/xeno/pounce]
	if(!pounce_ability)
		return
...
/// Returns the amount of AP that a maximum powered stealth will get.
/datum/mutation_upgrade/offense/maul/proc/get_multiplier(structure_count, include_initial = TRUE)
...
/datum/mutation_upgrade/utility/one_target/get_desc_for_alert(new_amount)
	if(!new_amount)
		return ..()
...
/datum/mutation_upgrade/utility/one_target/on_gain()
	. = ..()
...
/datum/mutation_upgrade/utility/one_target/on_loss()
	. = ..()
	var/datum/action/ability/activable/xeno/silence/silence_ability = xenomorph_owner.actions_by_path[/datum/action/ability/activable/xeno/silence]
	if(!silence_ability)
		return
...
/datum/mutation_upgrade/utility/one_target/on_structure_update(previous_amount, new_amount)
	. = ..()
	var/datum/action/ability/activable/xeno/silence/silence_ability = xenomorph_owner.actions_by_path[/datum/action/ability/activable/xeno/silence]
	if(!silence_ability)
		return
...
/datum/mutation_upgrade/utility/one_target/proc/get_multiplier(structure_count, include_initial = TRUE)
	return (include_initial ? multiplier_initial : 0) + (multiplier_per_structure * structure_count)
...
/datum/mutation_upgrade/utility/mirage_flood/get_desc_for_alert(new_amount)
	if(!new_amount)
		return ..()
	return "Mirage creates 4 additional illusions, but the duration is reduced by [get_duration(new_amount) / 10] seconds."

/datum/mutation_upgrade/utility/mirage_flood/on_gain()
	. = ..()
	var/datum/action/ability/xeno_action/mirage/mirage_ability = xenomorph_owner.actions_by_path[/datum/action/ability/xeno_action/mirage]
	if(!mirage_ability)
		return
...
/datum/mutation_upgrade/utility/mirage_flood/on_loss()
	. = ..()
	var/datum/action/ability/xeno_action/mirage/mirage_ability = xenomorph_owner.actions_by_path[/datum/action/ability/xeno_action/mirage]
	if(!mirage_ability)
		return
...
/datum/mutation_upgrade/utility/mirage_flood/on_structure_update(previous_amount, new_amount)
	. = ..()
	var/datum/action/ability/xeno_action/mirage/mirage_ability = xenomorph_owner.actions_by_path[/datum/action/ability/xeno_action/mirage]
	if(!mirage_ability)
		return
...
/datum/mutation_upgrade/utility/mirage_flood/proc/get_duration(structure_count, include_initial = TRUE)
...
/datum/mutation_upgrade/utility/faceblind/get_desc_for_alert(new_amount)
	if(!new_amount)
		return ..()
...
/datum/mutation_upgrade/utility/faceblind/on_gain()
	. = ..()
	var/datum/action/ability/xeno_action/stealth/stealth_ability = xenomorph_owner.actions_by_path[/datum/action/ability/xeno_action/stealth]
	if(!stealth_ability)
		return
...
/datum/mutation_upgrade/utility/faceblind/on_loss()
	. = ..()
	var/datum/action/ability/xeno_action/stealth/stealth_ability = xenomorph_owner.actions_by_path[/datum/action/ability/xeno_action/stealth]
	if(!stealth_ability)
		return
...
/datum/mutation_upgrade/utility/faceblind/on_structure_update(previous_amount, new_amount)
	. = ..()
	var/datum/action/ability/xeno_action/mirage/mirage_ability = xenomorph_owner.actions_by_path[/datum/action/ability/xeno_action/mirage]
	if(!mirage_ability)
		return
...
/datum/mutation_upgrade/utility/faceblind/proc/get_multiplier(structure_count)
