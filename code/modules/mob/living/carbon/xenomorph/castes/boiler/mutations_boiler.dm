...
/datum/mutation_upgrade/defense/staggered_panic/get_desc_for_alert(new_amount)
	if(!new_amount)
		return ..()
...
/datum/mutation_upgrade/defense/staggered_panic/on_gain()
...
/datum/mutation_upgrade/defense/staggered_panic/on_loss()
...
/datum/mutation_upgrade/defense/staggered_panic/proc/on_update_health(datum/source)
	SIGNAL_HANDLER
...
/datum/mutation_upgrade/defense/staggered_panic/proc/on_staggered(datum/source, amount, ignore_canstun)
...
/datum/mutation_upgrade/defense/staggered_panic/proc/get_globs(structure_count)
...
/datum/mutation_upgrade/defense/thick_containment/get_desc_for_alert(new_amount)
	if(!new_amount)
		return ..()
...
/datum/mutation_upgrade/defense/thick_containment/on_gain()
	xenomorph_owner.glob_luminosity_slowing += get_slowdown(0)
	return ..()

/datum/mutation_upgrade/defense/thick_containment/on_loss()
	xenomorph_owner.glob_luminosity_slowing -= get_slowdown(0)
	return ..()

/datum/mutation_upgrade/defense/thick_containment/on_structure_update(previous_amount, new_amount)
...
/datum/mutation_upgrade/defense/thick_containment/proc/get_slowdown(structure_count, include_initial = TRUE)
...
/datum/mutation_upgrade/defense/dim_containment/get_desc_for_alert(new_amount)
	if(!new_amount)
		return ..()
	return "The threshold for having excess globs is increased by [get_globs(new_amount)]."

/datum/mutation_upgrade/defense/dim_containment/on_structure_update(previous_amount, new_amount)
...
/datum/mutation_upgrade/defense/dim_containment/proc/get_globs(structure_count)
...
/datum/mutation_upgrade/offense/gaseous_spray/get_desc_for_alert(new_amount)
	if(!new_amount)
		return ..()
...
/datum/mutation_upgrade/offense/gaseous_spray/on_gain()
	. = ..()
...
/datum/mutation_upgrade/offense/gaseous_spray/on_loss()
	. = ..()
	var/datum/action/ability/activable/xeno/spray_acid/line/boiler/spray_ability = xenomorph_owner.actions_by_path[/datum/action/ability/activable/xeno/spray_acid/line/boiler]
	if(!spray_ability)
		return
...
/datum/mutation_upgrade/offense/gaseous_spray/on_structure_update(previous_amount, new_amount)
	. = ..()
	var/datum/action/ability/activable/xeno/spray_acid/line/boiler/spray_ability = xenomorph_owner.actions_by_path[/datum/action/ability/activable/xeno/spray_acid/line/boiler]
	if(!spray_ability)
		return
...
/// Returns the amount of stored globs that the owner needs to have in order to gain its effect.
/datum/mutation_upgrade/offense/gaseous_spray/proc/get_globs(structure_count, include_initial = TRUE)
...
/datum/mutation_upgrade/offense/hip_fire/get_desc_for_alert(new_amount)
	if(!new_amount)
		return ..()
...
/datum/mutation_upgrade/offense/hip_fire/on_gain()
	. = ..()
...
/datum/mutation_upgrade/offense/hip_fire/on_loss()
	var/datum/action/ability/activable/xeno/bombard/bombard_ability = xenomorph_owner.actions_by_path[/datum/action/ability/activable/xeno/bombard]
	if(!bombard_ability)
		return
	var/datum/action/ability/xeno_action/toggle_long_range/range_ability = xenomorph_owner.actions_by_path[/datum/action/ability/xeno_action/toggle_long_range]
...
/datum/mutation_upgrade/offense/hip_fire/on_structure_update(previous_amount, new_amount)
	. = ..()
	var/datum/action/ability/activable/xeno/bombard/bombard_ability = xenomorph_owner.actions_by_path[/datum/action/ability/activable/xeno/bombard]
	if(!bombard_ability)
		return
...
/datum/mutation_upgrade/offense/hip_fire/on_xenomorph_upgrade()
	var/datum/action/ability/xeno_action/toggle_long_range/range_ability = xenomorph_owner.actions_by_path[/datum/action/ability/xeno_action/toggle_long_range]
	if(range_ability)
...
/datum/mutation_upgrade/offense/hip_fire/proc/get_multiplier(structure_count, include_initial = TRUE)
...
/datum/mutation_upgrade/offense/rapid_fire/get_desc_for_alert(new_amount)
	if(!new_amount)
		return ..()
...
/datum/mutation_upgrade/offense/rapid_fire/on_gain()
	. = ..()
...
/datum/mutation_upgrade/offense/rapid_fire/on_loss()
	var/datum/action/ability/xeno_action/toggle_bomb/toggle_ability = xenomorph_owner.actions_by_path[/datum/action/ability/xeno_action/toggle_bomb]
	if(!toggle_ability)
		return
	var/datum/action/ability/activable/xeno/bombard/bombard_ability = xenomorph_owner.actions_by_path[/datum/action/ability/activable/xeno/bombard]
...
/datum/mutation_upgrade/offense/rapid_fire/on_structure_update(previous_amount, new_amount)
	. = ..()
	var/datum/action/ability/xeno_action/toggle_bomb/toggle_ability = xenomorph_owner.actions_by_path[/datum/action/ability/xeno_action/toggle_bomb]
	if(!toggle_ability)
		return
...
/datum/mutation_upgrade/offense/rapid_fire/proc/get_multiplier(structure_count, include_initial = TRUE)
	return (include_initial ? multiplier_initial : 0) + (multiplier_per_structure * structure_count)
...
/datum/mutation_upgrade/utility/acid_trail/get_desc_for_alert(new_amount)
	if(!new_amount)
		return ..()
	return "Whenever you move while carrying [get_globs(new_amount)] stored globs, an acid splatter is created underneath you."

/datum/mutation_upgrade/utility/acid_trail/on_gain()
	RegisterSignal(xenomorph_owner, COMSIG_MOVABLE_MOVED, PROC_REF(on_movement))
	return ..()

/datum/mutation_upgrade/utility/acid_trail/on_loss()
	UnregisterSignal(xenomorph_owner, COMSIG_MOVABLE_MOVED)
	return ..()

/datum/mutation_upgrade/utility/acid_trail/proc/on_movement(datum/source, atom/old_loc, movement_dir, forced, list/old_locs)
...
/// Returns the amount of stored globs that the owner needs to have in order to gain its effect.
/datum/mutation_upgrade/utility/acid_trail/proc/get_globs(structure_count, include_initial = TRUE)
	return (include_initial ? globs_initial : 0) + (globs_per_structure * structure_count)
...
/datum/mutation_upgrade/utility/chemical_mixing/get_desc_for_alert(new_amount)
	if(!new_amount)
		return ..()
...
/datum/mutation_upgrade/utility/chemical_mixing/on_gain()
	. = ..()
	var/datum/action/ability/activable/xeno/bombard/bombard_ability = xenomorph_owner.actions_by_path[/datum/action/ability/activable/xeno/bombard]
	if(!bombard_ability)
		return
...
/datum/mutation_upgrade/utility/chemical_mixing/on_loss()
	. = ..()
	var/datum/action/ability/activable/xeno/bombard/bombard_ability = xenomorph_owner.actions_by_path[/datum/action/ability/activable/xeno/bombard]
	if(!bombard_ability)
		return
...
/datum/mutation_upgrade/utility/chemical_mixing/on_structure_update(previous_amount, new_amount)
	. = ..()
	var/datum/action/ability/activable/xeno/bombard/bombard_ability = xenomorph_owner.actions_by_path[/datum/action/ability/activable/xeno/bombard]
	if(!bombard_ability)
		return
...
/// Returns the amount of stored globs that the owner needs to have in order to gain its effect.
/datum/mutation_upgrade/utility/chemical_mixing/proc/get_globs(structure_count, include_initial = TRUE)
	return (include_initial ? globs_initial : 0) + (globs_per_structure * structure_count)
...
/datum/mutation_upgrade/utility/binoculars/get_desc_for_alert(new_amount)
	if(!new_amount)
		return ..()
...
/datum/mutation_upgrade/utility/binoculars/on_gain()
	. = ..()
	var/datum/action/ability/xeno_action/toggle_long_range/range_ability = xenomorph_owner.actions_by_path[/datum/action/ability/xeno_action/toggle_long_range]
	if(!range_ability)
		return
...
/datum/mutation_upgrade/utility/binoculars/on_loss()
	. = ..()
	var/datum/action/ability/xeno_action/toggle_long_range/range_ability = xenomorph_owner.actions_by_path[/datum/action/ability/xeno_action/toggle_long_range]
	if(!range_ability)
		return
...
/datum/mutation_upgrade/utility/binoculars/on_structure_update(previous_amount, new_amount)
	. = ..()
	var/datum/action/ability/xeno_action/toggle_long_range/range_ability = xenomorph_owner.actions_by_path[/datum/action/ability/xeno_action/toggle_long_range]
	if(!range_ability)
		return
...
/datum/mutation_upgrade/utility/binoculars/proc/get_range(structure_count)
