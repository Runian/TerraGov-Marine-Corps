...
/datum/mutation_upgrade/defense/gaseous_trail/get_desc_for_alert(new_amount)
	if(!new_amount)
		return ..()
	return "Acid Dash leaves a trail of opaque gas behind you. The gas lasts for [get_duration(new_amount) * 0.1] seconds."

/datum/mutation_upgrade/defense/gaseous_trail/on_structure_update(previous_amount, new_amount)
	. = ..()
...
/datum/mutation_upgrade/defense/gaseous_trail/proc/get_duration(structure_count)
...
/datum/mutation_upgrade/offense/neurotoxin_swap/get_desc_for_alert(new_amount)
	if(!new_amount)
		return ..()
...
/datum/mutation_upgrade/offense/neurotoxin_swap/on_gain()
...
/datum/mutation_upgrade/offense/neurotoxin_swap/on_loss()
	var/datum/action/ability/activable/xeno/smokescreen_spit/smokescreen_ability = xenomorph_owner.actions_by_path[/datum/action/ability/activable/xeno/smokescreen_spit]
	if(!smokescreen_ability)
		return
...
/datum/mutation_upgrade/offense/neurotoxin_swap/on_structure_update(previous_amount, new_amount)
	. = ..()
	var/datum/action/ability/activable/xeno/smokescreen_spit/smokescreen_ability = xenomorph_owner.actions_by_path[/datum/action/ability/activable/xeno/smokescreen_spit]
	if(!smokescreen_ability)
		return
...
/datum/mutation_upgrade/offense/neurotoxin_swap/proc/get_cost_multiplier(structure_count, include_initial = TRUE)
...
/datum/mutation_upgrade/utility/fast_acid/get_desc_for_alert(new_amount)
	if(!new_amount)
		return ..()
	return "Corrosive Acid is now applied [PERCENT(get_speedup(new_amount))]% faster."

/datum/mutation_upgrade/utility/fast_acid/on_structure_update(previous_amount, new_amount)
...
/datum/mutation_upgrade/utility/fast_acid/proc/get_speedup(structure_count)
