...
/datum/mutation_upgrade/spur/drone/follow_up_slashes
	name = "Follow-Up Slashes"
...
/datum/mutation_upgrade/spur/drone/follow_up_slashes/on_mutation_enabled()
	RegisterSignal(xenomorph_owner, COMSIG_XENO_ESSENCE_LINK_TOGGLED, PROC_REF(on_essence_link_toggle))
	var/datum/action/ability/activable/xeno/essence_link/ability = xenomorph_owner.actions_by_path[/datum/action/ability/activable/xeno/essence_link]
	if(ability?.existing_link && ability.existing_link.current_beam)
		on_essence_link_toggle(xenomorph_owner, ability.existing_link.link_target, TRUE)
...
/datum/mutation_upgrade/spur/drone/follow_up_slashes/on_mutation_disabled()
	UnregisterSignal(xenomorph_owner, COMSIG_XENO_ESSENCE_LINK_TOGGLED)
	var/datum/action/ability/activable/xeno/essence_link/ability = xenomorph_owner.actions_by_path[/datum/action/ability/activable/xeno/essence_link]
	if(ability?.existing_link && ability.existing_link.current_beam)
		on_essence_link_toggle(xenomorph_owner, ability.existing_link.link_target, FALSE)
...
/datum/mutation_upgrade/spur/drone/follow_up_slashes/proc/remove_mark()
	if(!latest_marked_human || !timer_id)
		return
	latest_marked_human.remove_filter("follow_up_slashes_outline")
	latest_marked_human = null
	deltimer(timer_id)
	timer_id = null

/datum/mutation_upgrade/spur/drone/follow_up_slashes/proc/set_marked(mob/living/carbon/human/next_marked_human)
	remove_mark()
	latest_marked_human = next_marked_human
	latest_marked_human.add_filter("follow_up_slashes_outline", 2, outline_filter(1, COLOR_LIGHT_ORANGE))
	timer_id = addtimer(CALLBACK(src, PROC_REF(remove_mark)), 1 SECONDS, TIMER_UNIQUE|TIMER_STOPPABLE)

/datum/mutation_upgrade/spur/drone/follow_up_slashes/proc/on_essence_link_toggle(datum/source, mob/living/carbon/xenomorph/link_partner, toggled)
	if(toggled)
...
/datum/mutation_upgrade/spur/drone/follow_up_slashes/proc/on_attack_living_owner(datum/source, mob/living/target, damage, list/damage_mod, list/armor_mod)
	if(!ishuman(target))
		return
...
/datum/mutation_upgrade/spur/drone/follow_up_slashes/proc/on_attack_living_partner(datum/source, mob/living/target, damage, list/damage_mod, list/armor_mod)
...
/datum/mutation_upgrade/veil/drone/saving_grace
	...
	/// The percentage (as a decimal) to add to Acidic Salve's bonus heal maximum health threshold.
	var/bonus_heal_maximum_health_threshold_addition = 0.2
...
/datum/mutation_upgrade/veil/drone/saving_grace/on_mutation_enabled()
	...
	ability.bonus_heal_maximum_health_threshold += bonus_heal_maximum_health_threshold_addition
	return ..()
