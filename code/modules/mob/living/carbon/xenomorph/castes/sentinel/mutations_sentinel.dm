...
/datum/mutation_upgrade/defense/comforting_acid/get_desc_for_alert(new_amount)
	if(!new_amount)
		return ..()
...
/datum/mutation_upgrade/defense/comforting_acid/on_gain()
	. = ..()
...
/datum/mutation_upgrade/defense/comforting_acid/on_loss()
	. = ..()
	var/datum/action/ability/xeno_action/toxic_slash/slash_ability = xenomorph_owner.actions_by_path[/datum/action/ability/xeno_action/toxic_slash]
	if(!slash_ability)
		return
...
/datum/mutation_upgrade/defense/comforting_acid/on_structure_update(previous_amount, new_amount)
	. = ..()
	var/datum/action/ability/xeno_action/toxic_slash/slash_ability = xenomorph_owner.actions_by_path[/datum/action/ability/xeno_action/toxic_slash]
	if(!slash_ability)
		return
...
/datum/mutation_upgrade/defense/comforting_acid/proc/get_healing(structure_count, include_initial = TRUE)
...
/datum/mutation_upgrade/defense/healing_sting/get_desc_for_alert(new_amount)
	if(!new_amount)
		return ..()
...
/datum/mutation_upgrade/defense/healing_sting/on_structure_update(previous_amount, new_amount)
	. = ..()
	var/datum/action/ability/activable/xeno/drain_sting/sting_ability = xenomorph_owner.actions_by_path[/datum/action/ability/activable/xeno/drain_sting]
	if(!sting_ability)
		return
...
/datum/mutation_upgrade/defense/healing_sting/proc/get_multiplier(structure_count, include_initial = TRUE)
...
/datum/mutation_upgrade/defense/constant_surge/get_desc_for_alert(new_amount)
	if(!new_amount)
		return ..()
...
/datum/mutation_upgrade/defense/constant_surge/on_gain()
	. = ..()
	var/datum/action/ability/activable/xeno/drain_sting/sting_ability = xenomorph_owner.actions_by_path[/datum/action/ability/activable/xeno/drain_sting]
	if(!sting_ability)
		return
...
/datum/mutation_upgrade/defense/constant_surge/on_loss()
	. = ..()
	var/datum/action/ability/activable/xeno/drain_sting/sting_ability = xenomorph_owner.actions_by_path[/datum/action/ability/activable/xeno/drain_sting]
	if(!sting_ability)
		return
...
/datum/mutation_upgrade/defense/constant_surge/on_structure_update(previous_amount, new_amount)
	. = ..()
	var/datum/action/ability/activable/xeno/drain_sting/sting_ability = xenomorph_owner.actions_by_path[/datum/action/ability/activable/xeno/drain_sting]
	if(!sting_ability)
		return
...
/datum/mutation_upgrade/defense/constant_surge/proc/get_armor(structure_count, include_initial = TRUE)
...
/datum/mutation_upgrade/offense/acidic_slasher/get_desc_for_alert(new_amount)
	if(!new_amount)
		return ..()
...
/datum/mutation_upgrade/offense/acidic_slasher/on_gain()
...
/datum/mutation_upgrade/offense/acidic_slasher/on_loss()
	UnregisterSignal(xenomorph_owner, COMSIG_XENOMORPH_POSTATTACK_LIVING)
	xenomorph_owner.xeno_melee_damage_modifier += melee_damage_modifier
	return ..()

/datum/mutation_upgrade/offense/acidic_slasher/on_structure_update(previous_amount, new_amount)
...
/datum/mutation_upgrade/offense/acidic_slasher/proc/on_postattack(mob/living/source, mob/living/target, damage)
	SIGNAL_HANDLER
...
/datum/mutation_upgrade/offense/acidic_slasher/proc/get_move_adjust(structure_count)
...
/datum/mutation_upgrade/offense/acidic_slasher/proc/get_intoxicated_stacks(structure_count)
...
/datum/mutation_upgrade/offense/far_sting/get_desc_for_alert(new_amount)
	if(!new_amount)
		return ..()
...
/datum/mutation_upgrade/offense/far_sting/on_gain()
	. = ..()
	var/datum/action/ability/activable/xeno/drain_sting/sting_ability = xenomorph_owner.actions_by_path[/datum/action/ability/activable/xeno/drain_sting]
	if(!sting_ability)
		return
...
/datum/mutation_upgrade/offense/far_sting/on_loss()
	. = ..()
	var/datum/action/ability/activable/xeno/drain_sting/sting_ability = xenomorph_owner.actions_by_path[/datum/action/ability/activable/xeno/drain_sting]
	if(!sting_ability)
		return
...
/datum/mutation_upgrade/offense/far_sting/on_structure_update(previous_amount, new_amount)
	. = ..()
	var/datum/action/ability/activable/xeno/drain_sting/sting_ability = xenomorph_owner.actions_by_path[/datum/action/ability/activable/xeno/drain_sting]
	if(!sting_ability)
		return
...
/datum/mutation_upgrade/offense/far_sting/proc/get_effectiveness(structure_count, include_initial = TRUE)
...
/datum/mutation_upgrade/offense/imbued_claws/get_desc_for_alert(new_amount)
	if(!new_amount)
		return ..()
...
/datum/mutation_upgrade/offense/imbued_claws/on_gain()
	. = ..()
	var/datum/action/ability/activable/xeno/drain_sting/sting_ability = xenomorph_owner.actions_by_path[/datum/action/ability/activable/xeno/drain_sting]
	if(!sting_ability)
		return
...
/datum/mutation_upgrade/offense/imbued_claws/on_loss()
	. = ..()
	var/datum/action/ability/activable/xeno/drain_sting/sting_ability = xenomorph_owner.actions_by_path[/datum/action/ability/activable/xeno/drain_sting]
	if(!sting_ability)
		return
...
/datum/mutation_upgrade/offense/imbued_claws/on_structure_update(previous_amount, new_amount)
	. = ..()
	var/datum/action/ability/activable/xeno/drain_sting/sting_ability = xenomorph_owner.actions_by_path[/datum/action/ability/activable/xeno/drain_sting]
	if(!sting_ability)
		return
...
/datum/mutation_upgrade/offense/imbued_claws/proc/get_modifier(structure_count, include_initial = TRUE)
...
/datum/mutation_upgrade/utility/toxic_compatibility/get_desc_for_alert(new_amount)
	if(!new_amount)
		return ..()
...
/datum/mutation_upgrade/utility/toxic_compatibility/on_structure_update(previous_amount, new_amount)
	. = ..()
	var/datum/action/ability/activable/xeno/drain_sting/sting_ability = xenomorph_owner.actions_by_path[/datum/action/ability/activable/xeno/drain_sting]
	if(!sting_ability)
		return
...
/datum/mutation_upgrade/utility/toxic_compatibility/proc/get_amount(structure_count)
...
/datum/mutation_upgrade/utility/toxic_blood/get_desc_for_alert(new_amount)
	if(!new_amount)
		return ..()
...
/datum/mutation_upgrade/utility/toxic_blood/proc/on_damage(datum/source, amount, list/amount_mod)
	SIGNAL_HANDLER
...
/datum/mutation_upgrade/utility/toxic_blood/proc/get_damage_threshold(structure_count, include_initial = TRUE)
...
/datum/mutation_upgrade/utility/automatic_sting/get_desc_for_alert(new_amount)
	if(!new_amount)
		return ..()
...
/datum/mutation_upgrade/utility/automatic_sting/on_gain()
	. = ..()
	var/datum/action/ability/activable/xeno/drain_sting/sting_ability = xenomorph_owner.actions_by_path[/datum/action/ability/activable/xeno/drain_sting]
	if(!sting_ability)
		return
...
/datum/mutation_upgrade/utility/automatic_sting/on_loss()
	. = ..()
	var/datum/action/ability/activable/xeno/drain_sting/sting_ability = xenomorph_owner.actions_by_path[/datum/action/ability/activable/xeno/drain_sting]
	if(!sting_ability)
		return
...
/datum/mutation_upgrade/utility/automatic_sting/on_structure_update(previous_amount, new_amount)
	. = ..()
	var/datum/action/ability/activable/xeno/drain_sting/sting_ability = xenomorph_owner.actions_by_path[/datum/action/ability/activable/xeno/drain_sting]
	if(!sting_ability)
		return
...
/datum/mutation_upgrade/utility/automatic_sting/proc/on_postattack(mob/living/source, mob/living/target, damage)
	SIGNAL_HANDLER
...
/datum/mutation_upgrade/utility/automatic_sting/proc/get_stacks(structure_count, include_initial = TRUE)
