...
/datum/mutation_upgrade/defense/sentinel/armor_surge/on_loss()
	var/datum/action/ability/activable/xeno/drain_sting/ability = xenomorph_owner.actions_by_path[/datum/action/ability/activable/xeno/drain_sting]
	if(!ability)
		return
...
/datum/mutation_upgrade/defense/sentinel/healing_surge
	name = "Healing Surge"
	desc = "Drain Surge heals over time. Every second, it heals equal to the amount of bonus armor it gives."

/datum/mutation_upgrade/defense/sentinel/healing_surge/on_gain()
	var/datum/action/ability/activable/xeno/drain_sting/ability = xenomorph_owner.actions_by_path[/datum/action/ability/activable/xeno/drain_sting]
	if(!ability)
		return
	ability.drain_surge_heals_overtime = TRUE

/datum/mutation_upgrade/defense/sentinel/healing_surge/on_loss()
	var/datum/action/ability/activable/xeno/drain_sting/ability = xenomorph_owner.actions_by_path[/datum/action/ability/activable/xeno/drain_sting]
	if(!ability)
		return
...
/datum/mutation_upgrade/offense/sentinel/toxic_blood
	name = "Toxic Blood"
...
/datum/mutation_upgrade/offense/sentinel/toxic_blood/proc/on_damage_taken(datum/source, amount, list/amount_mod)
	SIGNAL_HANDLER
...
/datum/mutation_upgrade/offense/sentinel/toxic_claws
	name = "Toxic Claws"
...
/datum/mutation_upgrade/offense/sentinel/toxic_claws/on_gain()
...
/datum/mutation_upgrade/offense/sentinel/toxic_claws/on_loss()
...
/datum/mutation_upgrade/offense/sentinel/toxic_claws/on_xenomorph_upgrade()
	var/datum/action/ability/xeno_action/toxic_slash/ability = xenomorph_owner.actions_by_path[/datum/action/ability/xeno_action/toxic_slash]
	if(ability)
		ability.remove_action(xenomorph_owner)

/// Applies a variable amount of Intoxicated stacks to those that they attack.
/datum/mutation_upgrade/offense/sentinel/toxic_claws/proc/on_postattack(mob/living/source, mob/living/target, damage)
	SIGNAL_HANDLER
...
/datum/mutation_upgrade/offense/sentinel/claw_surge
	name = "Claw Surge"
...
/datum/mutation_upgrade/offense/sentinel/claw_surge/on_gain()
	var/datum/action/ability/activable/xeno/drain_sting/ability = xenomorph_owner.actions_by_path[/datum/action/ability/activable/xeno/drain_sting]
	if(!ability)
		return
	ability.drain_surge_modifier_instead = TRUE

/datum/mutation_upgrade/offense/sentinel/claw_surge/on_loss()
	var/datum/action/ability/activable/xeno/drain_sting/ability = xenomorph_owner.actions_by_path[/datum/action/ability/activable/xeno/drain_sting]
	if(!ability)
		return
...
/datum/mutation_upgrade/utility/sentinel/toxic_compatibility/on_gain()
	var/datum/action/ability/activable/xeno/drain_sting/ability = xenomorph_owner.actions_by_path[/datum/action/ability/activable/xeno/drain_sting]
	if(!ability)
		return
...
/datum/mutation_upgrade/utility/sentinel/vaporous_toxins
...
/datum/mutation_upgrade/utility/sentinel/vaporous_toxins/on_gain()
	var/datum/action/ability/activable/xeno/drain_sting/ability = xenomorph_owner.actions_by_path[/datum/action/ability/activable/xeno/drain_sting]
	if(!ability)
		return
	ability.creates_smoke = TRUE

/datum/mutation_upgrade/utility/sentinel/vaporous_toxins/on_loss()
	var/datum/action/ability/activable/xeno/drain_sting/ability = xenomorph_owner.actions_by_path[/datum/action/ability/activable/xeno/drain_sting]
	if(!ability)
		return
	ability.creates_smoke = initial(ability.creates_smoke)

/datum/mutation_upgrade/utility/sentinel/baton_surge
	name = "Baton Surge"
	desc = "Drain Sting can target xenomorphs. When used on xenomorphs, it immediately grants them Drain Surge."

/datum/mutation_upgrade/utility/sentinel/baton_surge/on_gain()
	var/datum/action/ability/activable/xeno/drain_sting/ability = xenomorph_owner.actions_by_path[/datum/action/ability/activable/xeno/drain_sting]
	if(!ability)
		return
	ability.can_target_allies = TRUE

/datum/mutation_upgrade/utility/sentinel/baton_surge/on_loss()
	var/datum/action/ability/activable/xeno/drain_sting/ability = xenomorph_owner.actions_by_path[/datum/action/ability/activable/xeno/drain_sting]
	if(!ability)
		return
