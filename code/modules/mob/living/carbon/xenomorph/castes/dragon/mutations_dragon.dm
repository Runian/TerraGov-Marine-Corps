...
/datum/mutation_upgrade/defense/regenerative_armor/get_desc_for_alert(new_amount)
	if(!new_amount)
		return ..()
...
/datum/mutation_upgrade/defense/regenerative_armor/on_gain()
	RegisterSignal(xenomorph_owner, COMSIG_XENOMORPH_HEALTH_REGEN, PROC_REF(on_health_regeneration))
	return ..()

/datum/mutation_upgrade/defense/regenerative_armor/on_loss()
...
/datum/mutation_upgrade/defense/regenerative_armor/proc/on_health_regeneration(datum/source, heal_data, seconds_per_tick)
	SIGNAL_HANDLER
...
/datum/mutation_upgrade/defense/regenerative_armor/proc/get_percentage(structure_count)
...
/datum/mutation_upgrade/offense/breath_of_variety/get_desc_for_alert(new_amount)
	if(!new_amount)
		return ..()
...
/datum/mutation_upgrade/offense/breath_of_variety/on_gain()
	. = ..()
...
/datum/mutation_upgrade/offense/breath_of_variety/on_loss()
	. = ..()
	var/datum/action/ability/activable/xeno/backhand/dragon_breath/breath_ability = xenomorph_owner.actions_by_path[/datum/action/ability/activable/xeno/backhand/dragon_breath]
	if(!breath_ability)
		return
...
/datum/mutation_upgrade/offense/breath_of_variety/on_structure_update(previous_amount, new_amount)
	. = ..()
	var/datum/action/ability/activable/xeno/backhand/dragon_breath/breath_ability = xenomorph_owner.actions_by_path[/datum/action/ability/activable/xeno/backhand/dragon_breath]
	if(!breath_ability)
		return
...
/datum/mutation_upgrade/offense/breath_of_variety/proc/get_multiplier(structure_count, include_initial = TRUE)
...
/datum/mutation_upgrade/utility/benevolence/get_desc_for_alert(new_amount)
	if(!new_amount)
		return ..()
	return "You emit all types of pheromones at [get_power(new_amount)] power in a radius of [get_radius(new_amount)]."

/datum/mutation_upgrade/utility/benevolence/on_structure_update(previous_amount, new_amount)
	. = ..()
	if(frenzy_aura)
...
/datum/mutation_upgrade/utility/benevolence/proc/get_power(structure_count)
	return power_increase_initial + (power_increase_per_structure * structure_count)

/// Returns the radius of the aura(s).
/datum/mutation_upgrade/utility/benevolence/proc/get_radius(power)
	return (6 + power) * 2
