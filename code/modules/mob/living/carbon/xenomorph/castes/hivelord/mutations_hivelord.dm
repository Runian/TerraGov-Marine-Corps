...
/datum/mutation_upgrade/defense/hardened_travel/get_desc_for_alert(new_amount)
	if(!new_amount)
		return ..()
	return "Resin Walk increases all soft armor by [get_armor(new_amount)], but prevents you from regenerating plasma."

/datum/mutation_upgrade/defense/hardened_travel/on_gain()
	. = ..()
...
/datum/mutation_upgrade/defense/hardened_travel/on_loss()
	. = ..()
	var/datum/action/ability/xeno_action/toggle_speed/speed_ability = xenomorph_owner.actions_by_path[/datum/action/ability/xeno_action/toggle_speed]
	if(!speed_ability)
		return
...
/datum/mutation_upgrade/defense/hardened_travel/on_structure_update(previous_amount, new_amount)
	. = ..()
	var/datum/action/ability/xeno_action/toggle_speed/speed_ability = xenomorph_owner.actions_by_path[/datum/action/ability/xeno_action/toggle_speed]
	if(!speed_ability)
		return
...
/datum/mutation_upgrade/defense/hardened_travel/proc/get_armor(structure_count, include_initial = TRUE)
...
/datum/mutation_upgrade/defense/costly_travel/get_desc_for_alert(new_amount)
	if(!new_amount)
		return ..()
	return "Resin Walk creates temporary weeds as you move. Each created weed consumes [get_plasma(new_amount)] plasma."

/datum/mutation_upgrade/defense/costly_travel/on_gain()
	. = ..()
	var/datum/action/ability/xeno_action/toggle_speed/speed_ability = xenomorph_owner.actions_by_path[/datum/action/ability/xeno_action/toggle_speed]
	if(!speed_ability)
		return
...
/datum/mutation_upgrade/defense/costly_travel/on_loss()
	. = ..()
	var/datum/action/ability/xeno_action/toggle_speed/speed_ability = xenomorph_owner.actions_by_path[/datum/action/ability/xeno_action/toggle_speed]
	if(!speed_ability)
		return
...
/datum/mutation_upgrade/defense/costly_travel/on_structure_update(previous_amount, new_amount)
	. = ..()
	var/datum/action/ability/xeno_action/toggle_speed/speed_ability = xenomorph_owner.actions_by_path[/datum/action/ability/xeno_action/toggle_speed]
	if(!speed_ability)
		return
...
/datum/mutation_upgrade/defense/costly_travel/proc/get_plasma(structure_count, include_initial = TRUE)
...
/datum/mutation_upgrade/defense/rejuvenating_build/get_desc_for_alert(new_amount)
	if(!new_amount)
		return ..()
	return "You heal [PERCENT(get_percentage(new_amount))]% of your maximum health whenever you successfully use Secrete Resin."

/datum/mutation_upgrade/defense/rejuvenating_build/on_structure_update(previous_amount, new_amount)
	. = ..()
...
/datum/mutation_upgrade/defense/rejuvenating_build/proc/get_percentage(structure_count)
...
/datum/mutation_upgrade/offense/resin_splash/get_desc_for_alert(new_amount)
	if(!new_amount)
		return ..()
...
/datum/mutation_upgrade/offense/resin_splash/on_gain()
	RegisterSignal(xenomorph_owner, COMSIG_XENOMORPH_ATTACK_HUMAN, PROC_REF(on_attack_human))
	return ..()

/datum/mutation_upgrade/offense/resin_splash/on_loss()
	UnregisterSignal(xenomorph_owner, COMSIG_XENOMORPH_ATTACK_HUMAN)
	return ..()

/datum/mutation_upgrade/offense/resin_splash/proc/on_attack_human(datum/source, mob/living/carbon/human/attacked_human)
	SIGNAL_HANDLER
...
/datum/mutation_upgrade/offense/resin_splash/proc/get_plasma(structure_count, include_initial = TRUE)
	return (include_initial ? plasma_initial : 0) + (plasma_per_structure * structure_count)
...
/datum/mutation_upgrade/offense/hostile_pylon/get_desc_for_alert(new_amount)
	if(!new_amount)
		return ..()
...
/datum/mutation_upgrade/offense/hostile_pylon/on_gain()
	. = ..()
...
/datum/mutation_upgrade/offense/hostile_pylon/on_loss()
	. = ..()
	var/datum/action/ability/xeno_action/place_recovery_pylon/pylon_ability = xenomorph_owner.actions_by_path[/datum/action/ability/xeno_action/place_recovery_pylon]
	if(!pylon_ability)
		return
...
/datum/mutation_upgrade/offense/hostile_pylon/on_structure_update(previous_amount, new_amount)
	. = ..()
	var/datum/action/ability/xeno_action/place_recovery_pylon/pylon_ability = xenomorph_owner.actions_by_path[/datum/action/ability/xeno_action/place_recovery_pylon]
	if(!pylon_ability)
		return
...
/datum/mutation_upgrade/offense/hostile_pylon/proc/get_modifier(structure_count)
...
/datum/mutation_upgrade/utility/protective_light/get_desc_for_alert(new_amount)
	if(!new_amount)
		return ..()
...
/datum/mutation_upgrade/utility/protective_light/on_gain()
	. = ..()
...
/datum/mutation_upgrade/utility/protective_light/on_loss()
	. = ..()
	var/datum/action/ability/activable/xeno/healing_infusion/healing_ability = xenomorph_owner.actions_by_path[/datum/action/ability/activable/xeno/healing_infusion]
	if(!healing_ability)
		return
...
/datum/mutation_upgrade/utility/protective_light/on_structure_update(previous_amount, new_amount)
	. = ..()
	var/datum/action/ability/activable/xeno/healing_infusion/healing_ability = xenomorph_owner.actions_by_path[/datum/action/ability/activable/xeno/healing_infusion]
	if(!healing_ability)
		return
...
/datum/mutation_upgrade/utility/protective_light/proc/get_multiplier(structure_count, include_initial = TRUE)
...
/datum/mutation_upgrade/utility/forward_light/get_desc_for_alert(new_amount)
	if(!new_amount)
		return ..()
...
/datum/mutation_upgrade/utility/forward_light/on_gain()
	. = ..()
	var/datum/action/ability/activable/xeno/healing_infusion/healing_ability = xenomorph_owner.actions_by_path[/datum/action/ability/activable/xeno/healing_infusion]
	if(!healing_ability)
		return
...
/datum/mutation_upgrade/utility/forward_light/on_loss()
	. = ..()
	var/datum/action/ability/activable/xeno/healing_infusion/healing_ability = xenomorph_owner.actions_by_path[/datum/action/ability/activable/xeno/healing_infusion]
	if(!healing_ability)
		return
...
/datum/mutation_upgrade/utility/forward_light/on_structure_update(previous_amount, new_amount)
	. = ..()
	var/datum/action/ability/activable/xeno/healing_infusion/healing_ability = xenomorph_owner.actions_by_path[/datum/action/ability/activable/xeno/healing_infusion]
	if(!healing_ability)
		return
...
/datum/mutation_upgrade/utility/forward_light/proc/get_multiplier(structure_count, include_initial = TRUE)
	return (include_initial ? multiplier_initial : 0) + (multiplier_per_structure * structure_count)
...
/datum/mutation_upgrade/utility/weed_specialist/get_desc_for_alert(new_amount)
	if(!new_amount)
		return ..()
...
/datum/mutation_upgrade/utility/weed_specialist/on_gain()
	. = ..()
...
/datum/mutation_upgrade/utility/weed_specialist/on_loss()
	. = ..()
	var/datum/action/ability/activable/xeno/plant_weeds/weed_ability = xenomorph_owner.actions_by_path[/datum/action/ability/activable/xeno/plant_weeds]
	if(!weed_ability)
		return
...
/datum/mutation_upgrade/utility/weed_specialist/on_structure_update(previous_amount, new_amount)
	. = ..()
	var/datum/action/ability/activable/xeno/plant_weeds/weed_ability = xenomorph_owner.actions_by_path[/datum/action/ability/activable/xeno/plant_weeds]
	if(!weed_ability)
		return
...
/datum/mutation_upgrade/utility/weed_specialist/proc/get_multiplier(structure_count, include_initial = TRUE)
	return (include_initial ? multiplier_initial : 0) + (multiplier_per_structure * structure_count)
