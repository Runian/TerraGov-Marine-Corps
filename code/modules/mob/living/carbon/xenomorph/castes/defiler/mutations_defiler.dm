...
/datum/mutation_upgrade/defense/panic_gas/get_desc_for_alert(new_amount)
	if(!new_amount)
		return ..()
...
/datum/mutation_upgrade/defense/panic_gas/on_gain()
...
/datum/mutation_upgrade/defense/panic_gas/on_loss()
...
/datum/mutation_upgrade/defense/panic_gas/proc/on_health_update(datum/source, amount, list/amount_mod)
	SIGNAL_HANDLER
...
/datum/mutation_upgrade/defense/panic_gas/proc/on_staggered(datum/source, amount, ignore_canstun)
...
/datum/mutation_upgrade/defense/panic_gas/proc/get_percentage(structure_count, include_initial = TRUE)
...
/datum/mutation_upgrade/offense/envenomed/get_desc_for_alert(new_amount)
	if(!new_amount)
		return ..()
...
/datum/mutation_upgrade/offense/envenomed/on_gain()
...
/datum/mutation_upgrade/offense/envenomed/on_loss()
	var/datum/action/ability/xeno_action/reagent_slash/slash_ability = xenomorph_owner.actions_by_path[/datum/action/ability/xeno_action/reagent_slash]
	if(!slash_ability)
		slash_ability = new()
...
/datum/mutation_upgrade/offense/envenomed/on_xenomorph_upgrade()
	var/datum/action/ability/xeno_action/reagent_slash/slash_ability = xenomorph_owner.actions_by_path[/datum/action/ability/xeno_action/reagent_slash]
	if(slash_ability)
		slash_ability.remove_action(xenomorph_owner)
		qdel(slash_ability)
...
/datum/mutation_upgrade/offense/envenomed/proc/on_selected_reagent(datum/source, datum/reagent/old_reagent_typepath, datum/reagent/new_reagent_typepath)
	SIGNAL_HANDLER
	if(particle_holder)
		QDEL_NULL(particle_holder)
...
/datum/mutation_upgrade/offense/envenomed/proc/on_attack_living(datum/source, mob/living/attacked_mob, damage, damage_mod, armor_mod)
	SIGNAL_HANDLER
...
/datum/mutation_upgrade/offense/envenomed/proc/get_percentage(structure_count, include_initial = TRUE)
	return (include_initial ? percentage_initial : 0) + (percentage_per_structure * structure_count)
...
/datum/mutation_upgrade/utility/wide_gas/get_desc_for_alert(new_amount)
	if(!new_amount)
		return ..()
...
/datum/mutation_upgrade/utility/wide_gas/on_gain()
	. = ..()
...
/datum/mutation_upgrade/utility/wide_gas/on_loss()
	. = ..()
	var/datum/action/ability/xeno_action/emit_neurogas/gas_ability = xenomorph_owner.actions_by_path[/datum/action/ability/xeno_action/emit_neurogas]
	if(!gas_ability)
		return FALSE
...
/datum/mutation_upgrade/utility/wide_gas/on_structure_update(previous_amount, new_amount)
	. = ..()
	var/datum/action/ability/xeno_action/emit_neurogas/gas_ability = xenomorph_owner.actions_by_path[/datum/action/ability/xeno_action/emit_neurogas]
	if(!gas_ability)
		return
...
/datum/mutation_upgrade/utility/wide_gas/proc/get_radius(structure_count, include_initial = TRUE)
