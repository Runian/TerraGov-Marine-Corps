...
/datum/mutation_upgrade/defense/flesh_for_life/proc/on_damage(datum/source, amount, list/amount_mod)
	SIGNAL_HANDLER
...
/datum/mutation_upgrade/defense/flesh_for_life/proc/get_plasma_per_damage(structure_count)
...
/datum/mutation_upgrade/offense/suffocating_presence/get_desc_for_alert(new_amount)
	if(!new_amount)
		return ..()
...
/datum/mutation_upgrade/offense/suffocating_presence/on_gain()
	. = ..()
...
/datum/mutation_upgrade/offense/suffocating_presence/on_loss()
	. = ..()
	var/datum/action/ability/xeno_action/dreadful_presence/dreadful_ability = xenomorph_owner.actions_by_path[/datum/action/ability/xeno_action/dreadful_presence]
	if(!dreadful_ability)
		return
...
/datum/mutation_upgrade/offense/suffocating_presence/on_structure_update(previous_amount, new_amount)
	. = ..()
	var/datum/action/ability/xeno_action/dreadful_presence/dreadful_ability = xenomorph_owner.actions_by_path[/datum/action/ability/xeno_action/dreadful_presence]
	if(!dreadful_ability)
		return
...
/datum/mutation_upgrade/offense/suffocating_presence/proc/get_damage(structure_count, include_initial = TRUE)
...
/datum/mutation_upgrade/utility/shifting_costs/get_desc_for_alert(new_amount)
	if(!new_amount)
		return ..()
...
/datum/mutation_upgrade/utility/shifting_costs/on_gain()
	. = ..()
...
/datum/mutation_upgrade/utility/shifting_costs/on_loss()
	. = ..()
	var/datum/action/ability/activable/xeno/puppet/puppet_ability = xenomorph_owner.actions_by_path[/datum/action/ability/activable/xeno/puppet]
	if(!puppet_ability)
		return
...
/datum/mutation_upgrade/utility/shifting_costs/on_structure_update(previous_amount, new_amount)
	. = ..()
	var/datum/action/ability/activable/xeno/puppet_blessings/blessings_ability = xenomorph_owner.actions_by_path[/datum/action/ability/activable/xeno/puppet_blessings]
	if(!blessings_ability)
		return
...
/datum/mutation_upgrade/utility/shifting_costs/proc/get_blessings_multiplier(structure_count, include_initial = TRUE)
