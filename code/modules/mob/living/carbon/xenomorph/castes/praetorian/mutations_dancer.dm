...
/datum/mutation_upgrade/defense/flame_dance/get_desc_for_alert(new_amount)
	if(!new_amount)
		return ..()
...
/datum/mutation_upgrade/defense/flame_dance/on_gain()
	. = ..()
...
/datum/mutation_upgrade/defense/flame_dance/on_loss()
	. = ..()
	var/datum/action/ability/activable/xeno/tail_trip/trip_ability = xenomorph_owner.actions_by_path[/datum/action/ability/activable/xeno/tail_trip]
	if(!trip_ability)
		return
...
/datum/mutation_upgrade/defense/flame_dance/on_structure_update(previous_amount, new_amount)
	. = ..()
	if(!attached_armor)
		return
...
/datum/mutation_upgrade/defense/flame_dance/proc/get_armor(structure_count, include_initial = TRUE)
...
/datum/mutation_upgrade/offense/bob_and_weave/get_desc_for_alert(new_amount)
	if(!new_amount)
		return ..()
	return "Tail Hook now pushes humans away instead of pulling them in. It deals [get_damage(new_amount)] additional damage."

/datum/mutation_upgrade/offense/bob_and_weave/on_gain()
	. = ..()
	var/datum/action/ability/activable/xeno/tail_hook/hook_ability = xenomorph_owner.actions_by_path[/datum/action/ability/activable/xeno/tail_hook]
	if(!hook_ability)
		return
...
/datum/mutation_upgrade/offense/bob_and_weave/on_loss()
	. = ..()
	var/datum/action/ability/activable/xeno/tail_hook/hook_ability = xenomorph_owner.actions_by_path[/datum/action/ability/activable/xeno/tail_hook]
	if(!hook_ability)
		return
...
/datum/mutation_upgrade/offense/bob_and_weave/on_structure_update(previous_amount, new_amount)
	. = ..()
	var/datum/action/ability/activable/xeno/tail_hook/hook_ability = xenomorph_owner.actions_by_path[/datum/action/ability/activable/xeno/tail_hook]
	if(!hook_ability)
		return
...
/datum/mutation_upgrade/offense/bob_and_weave/proc/get_damage(structure_count)
...
/datum/mutation_upgrade/utility/eb_and_flow/get_desc_for_alert(new_amount)
	if(!new_amount)
		return ..()
...
/datum/mutation_upgrade/utility/eb_and_flow/on_gain()
	. = ..()
...
/datum/mutation_upgrade/utility/eb_and_flow/on_loss()
	. = ..()
	var/datum/action/ability/xeno_action/dodge/dodge_ability = xenomorph_owner.actions_by_path[/datum/action/ability/xeno_action/dodge]
	if(!dodge_ability)
		return
...
/datum/mutation_upgrade/utility/eb_and_flow/on_structure_update(previous_amount, new_amount)
	. = ..()
	var/datum/action/ability/xeno_action/dodge/dodge_ability = xenomorph_owner.actions_by_path[/datum/action/ability/xeno_action/dodge]
	if(!dodge_ability)
		return
...
/datum/mutation_upgrade/utility/eb_and_flow/proc/get_multiplier(structure_count, include_initial = TRUE)
