...
/datum/mutation_upgrade/defense/advance_away/get_desc_for_alert(new_amount)
	if(!new_amount)
		return ..()
	return "Upon using Advance, you move [get_speed(new_amount)] faster for 6 seconds."

/datum/mutation_upgrade/defense/advance_away/on_structure_update(previous_amount, new_amount)
	. = ..()
...
/datum/mutation_upgrade/defense/advance_away/proc/get_speed(structure_count)
...
/datum/mutation_upgrade/offense/wall_bang/get_desc_for_alert(new_amount)
	if(!new_amount)
		return ..()
...
/datum/mutation_upgrade/offense/wall_bang/on_gain()
	. = ..()
...
/datum/mutation_upgrade/offense/wall_bang/on_loss()
	. = ..()
	var/datum/action/ability/activable/xeno/oppressor/dislocate/dislocate_ability = xenomorph_owner.actions_by_path[/datum/action/ability/activable/xeno/oppressor/dislocate]
	if(!dislocate_ability)
		return
...
/datum/mutation_upgrade/offense/wall_bang/on_structure_update(previous_amount, new_amount)
	. = ..()
	var/datum/action/ability/activable/xeno/oppressor/dislocate/dislocate_ability = xenomorph_owner.actions_by_path[/datum/action/ability/activable/xeno/oppressor/dislocate]
	if(!dislocate_ability)
		return
...
/datum/mutation_upgrade/offense/wall_bang/proc/get_percentage(structure_count, include_initial = TRUE)
...
/datum/mutation_upgrade/utility/low_charge/get_desc_for_alert(new_amount)
	if(!new_amount)
		return ..()
...
/datum/mutation_upgrade/utility/low_charge/on_gain()
	. = ..()
	var/datum/action/ability/activable/xeno/oppressor/advance/advance_ability = xenomorph_owner.actions_by_path[/datum/action/ability/activable/xeno/oppressor/advance]
	if(!advance_ability)
		return
...
/datum/mutation_upgrade/utility/low_charge/on_loss()
	. = ..()
	var/datum/action/ability/activable/xeno/oppressor/advance/advance_ability = xenomorph_owner.actions_by_path[/datum/action/ability/activable/xeno/oppressor/advance]
	if(!advance_ability)
		return
...
/datum/mutation_upgrade/utility/low_charge/on_structure_update(previous_amount, new_amount)
	. = ..()
	var/datum/action/ability/activable/xeno/oppressor/advance/advance_ability = xenomorph_owner.actions_by_path[/datum/action/ability/activable/xeno/oppressor/advance]
	if(!advance_ability)
		return
...
/datum/mutation_upgrade/utility/low_charge/proc/get_multiplier(structure_count, include_initial = TRUE)
