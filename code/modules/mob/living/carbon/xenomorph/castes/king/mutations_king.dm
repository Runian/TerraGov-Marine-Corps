...
/datum/mutation_upgrade/defense/stone_armor/get_desc_for_alert(new_amount)
	if(!new_amount)
		return ..()
	return "Allies that are in range of Petrify are granted [get_armor(new_amount)] armor for the duration of it."

/datum/mutation_upgrade/defense/stone_armor/on_structure_update(previous_amount, new_amount)
	. = ..()
...
/datum/mutation_upgrade/defense/stone_armor/proc/get_armor(structure_count)
...
/datum/mutation_upgrade/offense/minion_king/get_desc_for_alert(new_amount)
	if(!new_amount)
		return ..()
...
/datum/mutation_upgrade/offense/minion_king/on_gain()
	. = ..()
...
/datum/mutation_upgrade/offense/minion_king/on_loss()
	. = ..()
	var/datum/action/ability/xeno_action/psychic_summon/summon_ability = xenomorph_owner.actions_by_path[/datum/action/ability/xeno_action/psychic_summon]
	if(!summon_ability)
		return
...
/datum/mutation_upgrade/offense/minion_king/on_structure_update(previous_amount, new_amount)
	. = ..()
	var/datum/action/ability/xeno_action/psychic_summon/summon_ability = xenomorph_owner.actions_by_path[/datum/action/ability/xeno_action/psychic_summon]
	if(!summon_ability)
		return
...
/datum/mutation_upgrade/offense/minion_king/proc/get_amount(structure_count, include_initial = TRUE)
...
/datum/mutation_upgrade/utility/widefall/get_desc_for_alert(new_amount)
	if(!new_amount)
		return ..()
...
/datum/mutation_upgrade/utility/widefall/on_gain()
	. = ..()
...
/datum/mutation_upgrade/utility/widefall/on_loss()
	. = ..()
	var/datum/action/ability/activable/xeno/nightfall/nightfall_ability = xenomorph_owner.actions_by_path[/datum/action/ability/activable/xeno/nightfall]
	if(!nightfall_ability)
		return
...
/datum/mutation_upgrade/utility/widefall/on_structure_update(previous_amount, new_amount)
	. = ..()
	var/datum/action/ability/activable/xeno/nightfall/nightfall_ability = xenomorph_owner.actions_by_path[/datum/action/ability/activable/xeno/nightfall]
	if(!nightfall_ability)
		return
...
/datum/mutation_upgrade/utility/widefall/proc/get_range(structure_count)
...
/datum/mutation_upgrade/utility/flarefall/get_desc_for_alert(new_amount)
	if(!new_amount)
		return ..()
...
/datum/mutation_upgrade/utility/flarefall/on_gain()
	. = ..()
	var/datum/action/ability/activable/xeno/nightfall/nightfall_ability = xenomorph_owner.actions_by_path[/datum/action/ability/activable/xeno/nightfall]
	if(!nightfall_ability)
		return
...
/datum/mutation_upgrade/utility/flarefall/on_loss()
	. = ..()
	var/datum/action/ability/activable/xeno/nightfall/nightfall_ability = xenomorph_owner.actions_by_path[/datum/action/ability/activable/xeno/nightfall]
	if(!nightfall_ability)
		return
...
/datum/mutation_upgrade/utility/flarefall/on_structure_update(previous_amount, new_amount)
	. = ..()
	var/datum/action/ability/activable/xeno/nightfall/nightfall_ability = xenomorph_owner.actions_by_path[/datum/action/ability/activable/xeno/nightfall]
	if(!nightfall_ability)
		return
...
/datum/mutation_upgrade/utility/flarefall/proc/get_range(structure_count, include_initial = TRUE)
...
/datum/mutation_upgrade/utility/flarefall/proc/get_flare_multiplier(structure_count, include_initial = TRUE)
