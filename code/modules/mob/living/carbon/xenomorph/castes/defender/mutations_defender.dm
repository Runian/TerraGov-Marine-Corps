...
/datum/mutation_upgrade/defense/carapace_waxing/get_desc_for_alert(new_amount)
	if(!new_amount)
		return ..()
...
/datum/mutation_upgrade/defense/carapace_waxing/on_structure_update(previous_amount, new_amount)
	. = ..()
	if(!.)
		return
...
/datum/mutation_upgrade/defense/carapace_waxing/proc/get_debuff_stacks(structure_count)
...
/datum/mutation_upgrade/defense/brittle_upclose/get_desc_for_alert(new_amount)
	if(!new_amount)
		return ..()
...
/datum/mutation_upgrade/defense/brittle_upclose/on_gain()
...
/datum/mutation_upgrade/defense/brittle_upclose/on_loss()
	xenomorph_owner.soft_armor = xenomorph_owner.soft_armor.modifyRating(get_melee_armor(0), get_bullet_armor(0))
...
/datum/mutation_upgrade/defense/brittle_upclose/on_structure_update(previous_amount, new_amount)
	. = ..()
	if(!.)
		return
...
/datum/mutation_upgrade/defense/brittle_upclose/proc/pre_projectile_hit(datum/source, atom/movable/projectile/proj, cardinal_move, uncrossing)
	SIGNAL_HANDLER
...
/datum/mutation_upgrade/defense/brittle_upclose/proc/get_bullet_armor(structure_count, include_initial = TRUE)
...
/datum/mutation_upgrade/defense/brittle_upclose/proc/get_melee_armor(structure_count, include_initial = TRUE)
...
/datum/mutation_upgrade/defense/carapace_regrowth/get_desc_for_alert(new_amount)
	if(!new_amount)
		return ..()
...
/datum/mutation_upgrade/defense/carapace_regrowth/on_gain()
	var/datum/action/ability/xeno_action/regenerate_skin/regenerate_skin = xenomorph_owner.actions_by_path[/datum/action/ability/xeno_action/regenerate_skin]
	if(!regenerate_skin)
		return
...
/datum/mutation_upgrade/defense/carapace_regrowth/on_loss()
	var/datum/action/ability/xeno_action/regenerate_skin/regenerate_skin = xenomorph_owner.actions_by_path[/datum/action/ability/xeno_action/regenerate_skin]
	if(!regenerate_skin)
		return
	regenerate_skin.heal_multiplier -= get_heal_multiplier(0)
...
/datum/mutation_upgrade/defense/carapace_regrowth/on_structure_update(previous_amount, new_amount)
	. = ..()
	if(!.)
		return
	var/datum/action/ability/xeno_action/regenerate_skin/regenerate_skin = xenomorph_owner.actions_by_path[/datum/action/ability/xeno_action/regenerate_skin]
...
/datum/mutation_upgrade/defense/carapace_regrowth/proc/get_heal_multiplier(structure_count, include_initial = TRUE)
...
/datum/mutation_upgrade/offense/breathtaking_spin/get_desc_for_alert(new_amount)
	if(!new_amount)
		return ..()
...
/datum/mutation_upgrade/offense/breathtaking_spin/on_gain()
...
/datum/mutation_upgrade/offense/breathtaking_spin/on_loss()
	var/datum/action/ability/xeno_action/tail_sweep/tail_sweep = xenomorph_owner.actions_by_path[/datum/action/ability/xeno_action/tail_sweep]
	if(!tail_sweep)
		return
	tail_sweep.damage_multiplier -= get_damage_multiplier(0)
...
/datum/mutation_upgrade/offense/breathtaking_spin/on_structure_update(previous_amount, new_amount)
	. = ..()
	if(!.)
		return
	var/datum/action/ability/xeno_action/tail_sweep/tail_sweep = xenomorph_owner.actions_by_path[/datum/action/ability/xeno_action/tail_sweep]
...
/datum/mutation_upgrade/offense/breathtaking_spin/proc/get_damage_multiplier(structure_count, include_initial = TRUE)
...
/datum/mutation_upgrade/offense/power_spin/get_desc_for_alert(new_amount)
	if(!new_amount)
		return ..()
...
/datum/mutation_upgrade/offense/power_spin/on_gain()
	var/datum/action/ability/xeno_action/tail_sweep/tail_sweep = xenomorph_owner.actions_by_path[/datum/action/ability/xeno_action/tail_sweep]
	if(!tail_sweep)
		return
...
/datum/mutation_upgrade/offense/power_spin/on_loss()
	var/datum/action/ability/xeno_action/tail_sweep/tail_sweep = xenomorph_owner.actions_by_path[/datum/action/ability/xeno_action/tail_sweep]
	if(!tail_sweep)
		return
	tail_sweep.knockback_distance -= knockback_amount
...
/datum/mutation_upgrade/offense/power_spin/on_structure_update(previous_amount, new_amount)
	. = ..()
	if(!.)
		return
	var/datum/action/ability/xeno_action/tail_sweep/tail_sweep = xenomorph_owner.actions_by_path[/datum/action/ability/xeno_action/tail_sweep]
...
/datum/mutation_upgrade/offense/power_spin/proc/get_stagger_duration(structure_count)
...
/datum/mutation_upgrade/offense/sharpening_claws/get_desc_for_alert(new_amount)
	if(!new_amount)
		return ..()
...
/datum/mutation_upgrade/offense/sharpening_claws/on_gain()
	RegisterSignal(xenomorph_owner, COMSIG_XENOMORPH_SUNDER_CHANGE, PROC_REF(on_sunder_change))
	return ..()

/datum/mutation_upgrade/offense/sharpening_claws/on_loss()
	UnregisterSignal(xenomorph_owner, COMSIG_XENOMORPH_SUNDER_CHANGE)
	return ..()

/datum/mutation_upgrade/offense/sharpening_claws/on_structure_update(previous_amount, new_amount)
	. = ..()
	if(!.)
		return
...
/datum/mutation_upgrade/offense/sharpening_claws/proc/on_sunder_change(datum/source, old_sunder, new_sunder)
	SIGNAL_HANDLER
...
/datum/mutation_upgrade/offense/sharpening_claws/proc/get_modifier(structure_count)
...
/datum/mutation_upgrade/utility/carapace_sweat/get_desc_for_alert(new_amount)
	if(!new_amount)
		return ..()
...
/datum/mutation_upgrade/utility/carapace_sweat/on_structure_update(previous_amount, new_amount)
	. = ..()
	var/datum/action/ability/xeno_action/regenerate_skin/regenerate_skin = xenomorph_owner.actions_by_path[/datum/action/ability/xeno_action/regenerate_skin]
	if(!regenerate_skin)
		return
...
/datum/mutation_upgrade/utility/carapace_sweat/proc/get_duration(structure_count)
...
/datum/mutation_upgrade/utility/slow_and_steady/get_desc_for_alert(new_amount)
	if(!new_amount)
		return ..()
...
/datum/mutation_upgrade/utility/slow_and_steady/on_gain()
...
/datum/mutation_upgrade/utility/slow_and_steady/on_loss()
	var/datum/action/ability/xeno_action/fortify/fortify = xenomorph_owner.actions_by_path[/datum/action/ability/xeno_action/fortify]
	if(!fortify)
		return
...
/datum/mutation_upgrade/utility/slow_and_steady/on_structure_update(previous_amount, new_amount)
	. = ..()
	var/datum/action/ability/xeno_action/fortify/fortify = xenomorph_owner.actions_by_path[/datum/action/ability/xeno_action/fortify]
	if(!fortify)
		return
...
/datum/mutation_upgrade/utility/slow_and_steady/proc/get_delay(structure_count, include_initial = TRUE)
...
/datum/mutation_upgrade/utility/carapace_sharing/get_desc_for_alert(new_amount)
	if(!new_amount)
		return ..()
...
/datum/mutation_upgrade/utility/carapace_sharing/on_structure_update(previous_amount, new_amount)
	. = ..()
	var/datum/action/ability/xeno_action/regenerate_skin/regenerate_skin = xenomorph_owner.actions_by_path[/datum/action/ability/xeno_action/regenerate_skin]
	if(!regenerate_skin)
		return
...
/datum/mutation_upgrade/utility/carapace_sharing/proc/get_multiplier(structure_count, include_initial = TRUE)
