...
/datum/mutation_upgrade/defense/acid_sweat/get_desc_for_alert(new_amount)
	if(!new_amount)
		return ..()
...
/datum/mutation_upgrade/defense/acid_sweat/on_gain()
	RegisterSignal(xenomorph_owner, COMSIG_LIVING_IGNITED, PROC_REF(on_ignited))
	try_extinguish()
	return ..()

/datum/mutation_upgrade/defense/acid_sweat/on_loss()
...
/datum/mutation_upgrade/defense/acid_sweat/proc/try_extinguish(timed_extinguished = FALSE)
	if(timer_id)
...
/datum/mutation_upgrade/defense/acid_sweat/proc/on_ignited(datum/source, fire_stacks)
	SIGNAL_HANDLER
	try_extinguish()

/// Returns the amount of plasma that will be used.
/datum/mutation_upgrade/defense/acid_sweat/proc/get_cost(structure_count)
...
/datum/mutation_upgrade/offense/hit_and_run/get_desc_for_alert(new_amount)
	if(!new_amount)
		return ..()
...
/datum/mutation_upgrade/offense/hit_and_run/on_gain()
...
/datum/mutation_upgrade/offense/hit_and_run/on_loss()
	var/datum/action/ability/activable/xeno/scatter_spit/scatter_ability = xenomorph_owner.actions_by_path[/datum/action/ability/activable/xeno/scatter_spit]
	if(!scatter_ability)
		return
	scatter_ability.cast_time -= initial(scatter_ability.cast_time) * get_cast_multiplier(0)
...
/datum/mutation_upgrade/offense/hit_and_run/on_structure_update(previous_amount, new_amount)
	. = ..()
	var/datum/action/ability/activable/xeno/scatter_spit/scatter_ability = xenomorph_owner.actions_by_path[/datum/action/ability/activable/xeno/scatter_spit]
	if(!scatter_ability)
		return
...
/datum/mutation_upgrade/offense/hit_and_run/proc/get_cast_multiplier(structure_count, include_initial = TRUE)
...
/datum/mutation_upgrade/utility/wet_claws/proc/on_postattack_living(mob/living/carbon/xenomorph/source, mob/living/target, damage)
	SIGNAL_HANDLER
	if(!target.on_fire)
		return
...
/// Returns the amount to multiply the Scatter Spit's initial cast time which will then be used to increase it.
/datum/mutation_upgrade/utility/wet_claws/proc/get_stacks(structure_count)
