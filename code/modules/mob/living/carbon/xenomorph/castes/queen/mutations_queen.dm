...
/datum/mutation_upgrade/defense/healthy_bulwark/get_desc_for_alert(new_amount)
	if(!new_amount)
		return ..()
...
/datum/mutation_upgrade/defense/healthy_bulwark/on_gain()
	. = ..()
...
/datum/mutation_upgrade/defense/healthy_bulwark/on_loss()
	. = ..()
	var/datum/action/ability/xeno_action/bulwark/bulwark_ability = xenomorph_owner.actions_by_path[/datum/action/ability/xeno_action/bulwark]
	if(!bulwark_ability)
		return
...
/datum/mutation_upgrade/defense/healthy_bulwark/on_structure_update(previous_amount, new_amount)
	. = ..()
	var/datum/action/ability/xeno_action/bulwark/bulwark_ability = xenomorph_owner.actions_by_path[/datum/action/ability/xeno_action/bulwark]
	if(!bulwark_ability)
		return
...
/datum/mutation_upgrade/defense/healthy_bulwark/proc/get_overheal(structure_count, include_initial = TRUE)
...
/datum/mutation_upgrade/defense/bulwark_zone/get_desc_for_alert(new_amount)
	if(!new_amount)
		return ..()
...
/datum/mutation_upgrade/defense/bulwark_zone/on_gain()
	. = ..()
	var/datum/action/ability/xeno_action/bulwark/bulwark_ability = xenomorph_owner.actions_by_path[/datum/action/ability/xeno_action/bulwark]
	if(!bulwark_ability)
		return
...
/datum/mutation_upgrade/defense/bulwark_zone/on_loss()
	. = ..()
	var/datum/action/ability/xeno_action/bulwark/bulwark_ability = xenomorph_owner.actions_by_path[/datum/action/ability/xeno_action/bulwark]
	if(!bulwark_ability)
		return
...
/datum/mutation_upgrade/defense/bulwark_zone/on_structure_update(previous_amount, new_amount)
	. = ..()
	var/datum/action/ability/xeno_action/bulwark/bulwark_ability = xenomorph_owner.actions_by_path[/datum/action/ability/xeno_action/bulwark]
	if(!bulwark_ability)
		return
...
/datum/mutation_upgrade/defense/bulwark_zone/proc/get_multiplier(structure_count, include_initial = TRUE)
...
/datum/mutation_upgrade/offense/alternative_sting/get_desc_for_alert(new_amount)
	if(!new_amount)
		return ..()
...
/datum/mutation_upgrade/offense/alternative_sting/on_gain()
...
/datum/mutation_upgrade/offense/alternative_sting/on_loss()
	var/datum/action/ability/activable/xeno/neurotox_sting/neurotox_ability = xenomorph_owner.actions_by_path[/datum/action/ability/activable/xeno/neurotox_sting]
	if(neurotox_ability)
...
/datum/mutation_upgrade/offense/alternative_sting/on_structure_update(previous_amount, new_amount)
	. = ..()
	var/datum/action/ability/activable/xeno/neurotox_sting/neurotox_ability = xenomorph_owner.actions_by_path[/datum/action/ability/activable/xeno/neurotox_sting]
	if(!neurotox_ability)
		return
...
/datum/mutation_upgrade/offense/alternative_sting/on_xenomorph_upgrade()
	var/datum/action/ability/activable/xeno/neurotox_sting/ozelomelyn/ozelomelyn_ability = xenomorph_owner.actions_by_path[/datum/action/ability/activable/xeno/neurotox_sting/ozelomelyn]
	if(ozelomelyn_ability)
		ozelomelyn_ability.remove_action(xenomorph_owner)
		qdel(ozelomelyn_ability)
...
/datum/mutation_upgrade/offense/alternative_sting/proc/get_multiplier(structure_count)
...
/datum/mutation_upgrade/utility/rallying_cry/get_desc_for_alert(new_amount)
	if(!new_amount)
		return ..()
	return "Screech grant all xenomorphs within a 20-tile radius gain [get_movement(new_amount)] speed for 4 seconds."

/datum/mutation_upgrade/utility/rallying_cry/on_gain()
	. = ..()
...
/datum/mutation_upgrade/utility/rallying_cry/on_loss()
	. = ..()
	var/datum/action/ability/activable/xeno/screech/screech_ability = xenomorph_owner.actions_by_path[/datum/action/ability/activable/xeno/screech]
	if(!screech_ability)
		return
...
/datum/mutation_upgrade/utility/rallying_cry/on_structure_update(previous_amount, new_amount)
	. = ..()
	var/datum/action/ability/activable/xeno/screech/screech_ability = xenomorph_owner.actions_by_path[/datum/action/ability/activable/xeno/screech]
	if(!screech_ability)
		return
...
/datum/mutation_upgrade/utility/rallying_cry/proc/get_movement(structure_count, include_initial = TRUE)
