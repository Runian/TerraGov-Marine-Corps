...
/datum/mutation_upgrade/defense/unstoppable/get_desc_for_alert(new_amount)
	if(!new_amount)
		return ..()
...
/datum/mutation_upgrade/defense/unstoppable/on_structure_update(previous_amount, new_amount)
	. = ..()
...
/datum/mutation_upgrade/defense/unstoppable/proc/get_steps(structure_count)
...
/datum/mutation_upgrade/offense/speed_demon/get_desc_for_alert(new_amount)
	if(!new_amount)
		return ..()
	return "Charging costs twice as much plasma. The speed multiplier per step is increased by [get_speed(new_amount)]."

/datum/mutation_upgrade/offense/speed_demon/on_gain()
	. = ..()
	var/datum/action/ability/xeno_action/ready_charge/bull_charge/charge_ability = xenomorph_owner.actions_by_path[/datum/action/ability/xeno_action/ready_charge/bull_charge]
	if(!charge_ability)
		return
...
/datum/mutation_upgrade/offense/speed_demon/on_loss()
	. = ..()
	var/datum/action/ability/xeno_action/ready_charge/bull_charge/charge_ability = xenomorph_owner.actions_by_path[/datum/action/ability/xeno_action/ready_charge/bull_charge]
	if(!charge_ability)
		return
...
/datum/mutation_upgrade/offense/speed_demon/on_structure_update(previous_amount, new_amount)
	. = ..()
	var/datum/action/ability/xeno_action/ready_charge/bull_charge/charge_ability = xenomorph_owner.actions_by_path[/datum/action/ability/xeno_action/ready_charge/bull_charge]
	if(!charge_ability)
		return
...
/datum/mutation_upgrade/offense/speed_demon/proc/get_speed(structure_count)
...
/datum/mutation_upgrade/utility/railgun/get_desc_for_alert(new_amount)
	if(!new_amount)
		return ..()
	return "Charge's maximum steps is increased by [get_steps(new_amount)]."

/datum/mutation_upgrade/utility/railgun/on_structure_update(previous_amount, new_amount)
	. = ..()
	var/datum/action/ability/xeno_action/ready_charge/bull_charge/charge_ability = xenomorph_owner.actions_by_path[/datum/action/ability/xeno_action/ready_charge/bull_charge]
	if(!charge_ability)
		return
...
/datum/mutation_upgrade/utility/railgun/proc/get_steps(structure_count)
