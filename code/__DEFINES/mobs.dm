...
#define CASTE_NO_HEALING (1<<2) // Xenomorphs that cannot heal by being on weeds or innately.

#define CASTE_INNATE_PLASMA_REGEN (1<<3) // Xenomorphs that regenerate plasma outside of weeds.
#define CASTE_PLASMADRAIN_IMMUNE (1<<4) // Are we immune to plasma drain?
...
#define CASTE_NO_PSYCHIC_SUMMON (1<<18) // Excluded from the King's psychic summon.
...
#define CASTE_HIDE_IN_STATUS (1<<20) // Do not count them in the hive status TGUI.
#define CASTE_NOT_IN_BIOSCAN (1<<21) // Do not count them toward the xenomorph count for the bioscan. Typically given to summoned minions (puppet/spiderling).
#define CASTE_HAS_WOUND_MASK (1<<22) // Uses an alpha mask for wounded states.

// Xenomorph caste_flags (evolution):
// NOTE: Consider making a new variable for these.
#define CASTE_EVOLUTION_ALLOWED (1<<23) // Are we allowed to evolve & do we gain any evolution points?
#define CASTE_INSTANT_EVOLUTION (1<<24) // Whether we require no evolution progress to evolve to this caste.
#define CASTE_CANNOT_EVOLVE_IN_CAPTIVITY (1<<25) // Whether we cannot evolve in the research lab.
#define CASTE_REQUIRES_FREE_TILE (1<<26) // Whether we require a free tile to evolve.
#define CASTE_LEADER_TYPE (1<<27) // Whether this is a leader type caste (e.g. Queen/Shrike/King/Dragon). Restricts who can play these castes based on: playtime & if banned from Queen.
#define CASTE_EXCLUDE_STRAINS (1<<28) // Excludes this caste/basetype from strain selection.
